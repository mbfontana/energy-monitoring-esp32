// includes necessários para funcionar
...
#define SWAP(a,b) tempr=(a); (a)=(b); (b)=tempr   // constante/função da rede elétrica
...
enum { BUF_LEN = 5*SAMPLES};     // máximo de valores no buffer circular (5 ciclos)
enum { MSG_LEN = 100 };          // máximo de caracteres no corpo de mensagem
enum { MSG_QUEUE_LEN = 5 };      // slots da fila de mensagens
enum { CMD_BUF_LEN = 255};       // número de caracteres no buffer de comandos
...
void maf(sMAF *m_a_f, double m_input, int n_samples)
{
  ...
  if (m_a_f->MAF_KONTz == n_samples) m_a_f->MAF_KONTz=0;
...
void rms(RMS *r_m_s, double RMS_input, int n_samples)
{
  ...
  if(r_m_s->inicio == 0)
  {
    r_m_s->temp = 0;
    r_m_s->NA = 0;
    for(i=0;i<CPT_COLETA;i++) {r_m_s->jm[i] = 0;}
  ...
  if (r_m_s->summed<=0) r_m_s->summed=0.0001;
...
void CPT(double ua, double ia)
{
  ...
  if (P.mean_out < 0.000001) P.mean_out=0.000001;
      ...
      if ((c == '\n') || (c == '\r')) {
        ...
        if (strcmp(cmd_buf, v_command3) == 0) {
          for(int aux = 0; aux<5;aux++)
          {
            ...
            {
              if(i%2==0) Serial.println((float)vComp[i]);
            }
        ...
        if (strcmp(cmd_buf, v_command5) == 0) {
          ...
          for(int aux = 0; aux<5;aux++)
          {
            for(int i=0;i<2*SAMPLES;i++)
            {
              ...
              if(i%2==0) sum_signal+=val_signal*val_signal;
            }
          }
        ...
        if (strcmp(cmd_buf, i_command3) == 0) {
          for(int aux = 0; aux<5;aux++)
          {
            for(int i=0;i<2*SAMPLES;i++)
            {
              if(i%2==0) Serial.println((float)iComp[i]);
            }
        ...
        if (strcmp(cmd_buf, etc_command) == 0) {
          for(int aux = 0; aux<5;aux++)
          {
            for(int i=0;i<2*SAMPLES;i++)
            {
              if(i%2==0) Serial.println((float)abs(vComp[i]));
            }
        ...
        if (strcmp(cmd_buf, etc_command1) == 0) {
          for(int aux = 0; aux<5;aux++)
          {
            for(int i=0;i<2*SAMPLES;i++)
            {
              if(i%2==0) Serial.println((float)abs(iComp[i]));
            }
